    ...
    <div className="card-md space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input ref={sigRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleSignatureUpload(f); }} />
          <div className="border border-slate-200 rounded-xl p-4 flex items-center justify-between gap-4 bg-slate-50">
            {form.signatureUrl ? (
              <React.Fragment>
                <img src={form.signatureUrl} alt="Signature" className="h-12 object-contain bg-white rounded p-1 border" />
                <div className="flex gap-2">
                  <button type="button" onClick={() => sigRef.current?.click()} className="btn-outline py-1.5 px-2.5 text-xs">
                    Change
                  </button>
                  <button type="button" onClick={handleDeleteSignature} className="btn-danger py-1.5 px-2.5 text-xs">
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
                </div>
              </React.Fragment>
            ) : (
              <button type="button" onClick={() => sigRef.current?.click()} className="btn-outline py-2 px-4 text-xs w-full text-center">
