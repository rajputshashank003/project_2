...
import { Users, Upload, Save, Trash2, UserCircle2 } from 'lucide-react';
...
const SlotCard: React.FC<{ slot: TeamMemberSlot }> = ({ slot }) => {
  ...
  if (!ctx) return null;
  const { forms, saving, clearing, setSlotField, handlePhotoUpload, handleSave, openClearConfirm } = ctx;
  const f = forms[slot];

  return (
    <div className="card-md flex flex-col gap-5">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-medium text-red-500 hover:text-red-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        >
...
const AdminTeamContent: React.FC = () => {
  const ctx = useContext(AdminTeamContext);
  if (!ctx) return null;
  const { isLoading, clearTarget, cancelClear, confirmClear, clearing } = ctx;
    ...
    <div className="page-wrapper">
      <div className="mb-8">
        ...
        <p className="section-subheading">
          Manage the 3 fixed team member slots shown on the public About page. Upload a photo, enter name and designation, then save.
        </p>
      </div>
      ...
      <ConfirmModal
        isOpen={!!clearTarget}
        title="Clear Slot?"
        message={`This will remove the photo, name, and designation for Slot ${clearTarget}. The slot itself remains on the About page with a placeholder. Continue?`}
        confirmText="Clear"
