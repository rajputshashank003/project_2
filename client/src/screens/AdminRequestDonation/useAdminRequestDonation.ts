...
export const useAdminRequestDonation = () => {
  ...
  const [screenshotModal, setScreenshotModal] = useState<Donation | null>(null);
  ...
  const filteredDonations = (() => {
    let list = filterStatus === 'all' ? donations : filter(donations, (d) => d.status === filterStatus);
    if (searchQuery.trim()) {
  ...
  const openApprove = (item: Donation) => { setActionItem(item); setActionType('approve'); setRejectReason(''); };
  const openReject  = (item: Donation) => { setActionItem(item); setActionType('reject');  setRejectReason(''); };
...
export type ReturnTypeOfUseAdminRequestDonation = ReturnType<typeof useAdminRequestDonation>;
