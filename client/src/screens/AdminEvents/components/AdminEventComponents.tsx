        ...
        <div className="page-wrapper">
            <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
                <div>
                    ...
                    <p className="section-subheading">
                        Add, edit, and remove events shown on the public Events
                        page
                    </p>
                </div>
                <button
            ...
            {isLoading ? (
                ...
                <div className="card text-center text-slate-400 py-16">
                    ...
                    <p>
                        No events yet. Click "Add Event" to create the first
                        one.
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {events.map((event) => (
                        <div
                            key={event.id}
                            className="bg-white rounded-3xl border border-slate-100 shadow-card hover:shadow-card-hover transition-all duration-300 overflow-hidden flex flex-col group"
                        >
                            ...
                            <div className="p-5 flex-1 flex flex-col justify-between">
                                <div>
                                    <h3 className="font-bold text-slate-900 text-base mb-1.5 leading-snug line-clamp-1">
                                        {event.title}
                                    </h3>
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        className={`form-input resize-none ${formErrors.description ? "border-red-400" : ""}`}
                    />
                    ...
                    {pendingImages.length === 0 ? (
                        <React.Fragment>
                            ...
                            <button
                                type="button"
                                onClick={() => fileRef.current?.click()}
                                className={`w-full h-32 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center gap-2 transition-colors ${formErrors.images ? "border-red-300 bg-red-50" : "border-slate-200 hover:border-emerald-400 hover:bg-emerald-50/50"}`}
                            >
                                <Upload
                                    className={`h-6 w-6 ${formErrors.images ? "text-red-400" : "text-slate-400"}`}
                                />
                                <span className="text-sm font-medium text-slate-600">
                                    Click to upload event photo
                                </span>
                                <span className="text-xs text-slate-400">
                                    JPG, PNG or WebP · Max 5MB · Up to 3 photos
                                </span>
                            </button>
                        </React.Fragment>
                    ) : (
                        <div className="grid grid-cols-3 gap-3">
                            {pendingImages.map((img, i) => (
                                <div
                                    key={i}
                                    className="relative aspect-square rounded-2xl overflow-hidden border border-slate-200 bg-slate-100 shadow-xs"
                                >
                                    <img
                                        src={
                                            img.type === "new"
                                    ...
                                    <button
                                        ...
                                        className="absolute top-2 right-2 p-1.5 rounded-full bg-slate-900/70 hover:bg-red-600 text-white transition-colors shadow-sm"
                                        title="Remove photo"
                                    >
                            ...
                            {pendingImages.length < MAX_IMAGES && (
                                <React.Fragment>
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => fileRef.current?.click()}
                                        className="aspect-square rounded-2xl border-2 border-dashed border-slate-200 hover:border-emerald-400 hover:bg-emerald-50 flex flex-col items-center justify-center gap-1 text-slate-400 hover:text-emerald-600 transition-colors"
                                    >
