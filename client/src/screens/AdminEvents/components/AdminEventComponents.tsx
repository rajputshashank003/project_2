    ...
    <div className="page-wrapper">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {events.map((event) => (
            <div key={event.id} className="bg-white rounded-2xl border border-slate-100 shadow-card overflow-hidden flex flex-col">
              <div className="relative aspect-video bg-slate-100">
                {event.images[0] ? (
                  <img src={event.images[0].imageUrl} alt={event.title} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="p-4 flex-1 flex flex-col">
                <h3 className="font-bold text-slate-900 mb-1 leading-snug">{event.title}</h3>
                <p className="text-xs text-slate-400 mb-3 line-clamp-2">{event.description}</p>
                <div className="mt-auto flex gap-2">
                  <button onClick={() => openEdit(event)} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors">
                    <Pencil className="h-3.5 w-3.5" /> Edit
                  </button>
                  <button onClick={() => openDeleteConfirm(event.id)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium bg-red-50 border border-red-200 text-red-600 hover:bg-red-100 transition-colors">
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button id="save-event-btn" onClick={handleSave} disabled={isSaving} className="btn-primary px-5 py-2">
            {isSaving ? (
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="form-label mb-0">Images * <span className="text-slate-400 font-normal">({pendingImages.length}/{MAX_IMAGES})</span></label>
            {pendingImages.length < MAX_IMAGES && (
              <>
                ...
                <button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 text-sm font-medium text-emerald-700 hover:text-emerald-900 transition-colors">
          ...
          {pendingImages.length === 0 ? (
            <>
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { handleAddImage(f); e.target.value = ''; } }} />
              <button type="button" onClick={() => fileRef.current?.click()} className={`w-full h-32 border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 transition-colors ${formErrors.images ? 'border-red-300 bg-red-50' : 'border-slate-200 hover:border-emerald-400 hover:bg-emerald-50/50'}`}>
                <Upload className={`h-6 w-6 ${formErrors.images ? 'text-red-400' : 'text-slate-400'}`} />
                <span className="text-sm text-slate-500">Click to upload image</span>
                <span className="text-xs text-slate-400">JPG, PNG or WebP · Max 5MB · Up to 3 images</span>
              </button>
            </>
          ) : (
            <div className="grid grid-cols-3 gap-3">
              {pendingImages.map((img, i) => (
                <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200 bg-slate-100">
                  ...
                  <button onClick={() => handleRemoveImage(i)} className="absolute top-1 right-1 p-1 rounded-full bg-red-600 text-white hover:bg-red-700 transition-colors">
                    <X className="h-3 w-3" />
                  </button>
                </div>
              ))}
              {pendingImages.length < MAX_IMAGES && (
                <>
                  <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { handleAddImage(f); e.target.value = ''; } }} />
                  <button type="button" onClick={() => fileRef.current?.click()} className="aspect-square rounded-xl border-2 border-dashed border-slate-200 hover:border-emerald-400 hover:bg-emerald-50 flex flex-col items-center justify-center gap-1 text-slate-400 hover:text-emerald-600 transition-colors">
