...
const LogoBadge: React.FC<{ logoUrl?: string; name: string; size?: 'sm' | 'md' }> = ({
  logoUrl,
  name,
  size = 'md',
}) => {
  ...
  return (
    ...
    >
      <Leaf className={`${size === 'sm' ? 'h-4 w-4' : 'h-5 w-5'} text-white`} />
    </div>
  );
};
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <Link to="/" className="flex items-center gap-3 group focus:outline-none min-h-[40px]">
              {isConfigLoading ? (
                ...
                <React.Fragment>
                  <LogoBadge logoUrl={ngoConfig.logoUrl} name={ngoConfig.name} size="md" />
              ...
              {isAdmin && (
                <MenuDropdown
                  ...
                  trigger={
                    ...
                    >
                      ...
                      Admin
                      <ChevronDown className={`h-3.5 w-3.5 text-emerald-700 transition-transform duration-200 ${adminOpen ? 'rotate-180' : ''}`} />
                    </button>
                  }
                  align="right"
                  className="w-56 py-2"
                >
                  <div className="px-3 py-1.5 mb-1 border-b border-slate-100">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Admin Control</span>
                  </div>
        ...
        {menuOpen &&
          createPortal(
            <div className="min-[1200px]:hidden fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true">
              {/* Backdrop */}
              <div
              ...
              <div className="relative w-full max-w-sm bg-white h-[100dvh] shadow-2xl flex flex-col z-10 animate-drawer-slide border-l border-slate-200">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50/80 shrink-0">
                  {isConfigLoading ? (
                    ...
                    <div className="flex items-center gap-2.5">
                      <LogoBadge logoUrl={ngoConfig.logoUrl} name={ngoConfig.name} size="sm" />
                      <div className="flex flex-col">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-emerald-100">Access ID cards &amp; receipts</div>
                        </div>
                      </div>
